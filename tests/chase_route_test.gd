...
## return door, which leads to the room behind — and the room behind is Level_10,
## a room he has already cleared. The escape actually continues into Level_12,
...
##   2. AFTER it, the same doorway lands him at Level_12's ENTRANCE.
...
func _ready() -> void:
	# Open in Level_10 rather than at the start of the Act: the opening cutscene
	# locks input and waits on a button press that headless will never send.
	LdtkWorld.debug_start_room = "Level_10"
	world = load("res://ldtk/Act1World.tscn").instantiate()
	add_child(world)
	await _frames(10)
	LdtkWorld.debug_start_room = ""

	from_room = _room("Level_10")
	chase_room = _room("Level_11")
	next_room = _room("Level_12")
	...
	_check(world.current_room == from_room,
		"opened in Level_10  [%s]" % world.current_room.name)
	...
	_check(trigger != null, "Level_11 has a DarkshangTrigger")
	if trigger == null:
		return _finish()
	...
	_check(world.current_room == chase_room,
		"walked into Level_11  [%s]" % world.current_room.name)
	await _go_back()
	_check(world.current_room == from_room,
		"untriggered, backing out of Level_11 still leads to Level_10  [%s]"
			% world.current_room.name)

	# --- 2. after it, the same doorway continues the escape -------------------
	await _go_forward()
	_check(world.current_room == chase_room, "back in Level_11 for the encounter")
	...
	_check(world.current_room == next_room,
		"after meeting him it leads to Level_12 instead  [%s]" % world.current_room.name)
	...
	_check(world._checkpoint.is_equal_approx(world.spawn_point_for(next_room)),
		"landing at Level_12's PlayerStart  [%s vs %s]"
			% [world._checkpoint, world.spawn_point_for(next_room)])

	# --- 3. and it holds, however many times the doorway is used --------------
	await _go_back()
	_check(world.current_room == chase_room,
		"Level_12 still backs out into Level_11  [%s]" % world.current_room.name)
	await _go_back()
	_check(world.current_room == next_room,
		"and leaving Level_11 a second time still goes to Level_12  [%s]"
			% world.current_room.name)

	# Level_10 is behind him for good, which is the point: the room he cleared on
	# the way in is not somewhere the chase can spill back into.
	await _go_back()
	_check(world._return_room == next_room,
		"the boss room's doorway no longer points at Level_10  [%s]"
	...
	# --- 4. the chase carries on past Level_12 -------------------------------
	# Level_13 sits to the LEFT of Level_12 on the bottom row, so world-sort order
	# runs ... Level_11, Level_13, Level_12 while the chase runs 11 -> 12 -> 13.
	...
	var last_room := _room("Level_13")
	_check(last_room != null, "Level_13 exists")
	if last_room != null:
		await _go_back()             # back into Level_12, facing its Exit
		_check(world.current_room == next_room, "back in Level_12")
		await _go_forward()
		_check(world.current_room == last_room,
			"Level_12's Exit reaches Level_13  [%s]" % world.current_room.name)
		await _go_back()
		_check(world.current_room == next_room,
			"and Level_13 backs out into Level_12  [%s]" % world.current_room.name)
...
## comes from the world, not from an assumption here — Level_12 sits to the LEFT
## of Level_11 despite being the room after it, so a hardcoded left edge would
