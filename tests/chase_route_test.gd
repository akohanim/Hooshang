...
func _ready() -> void:
	...
	_check(trigger != null, "Level_14 has a DarkshangTrigger")
	if trigger == null:
		return _finish()
	...
	await _go_forward()
	_check(world.current_room == chase_room, "back in Level_14 for the encounter")
	...
	#
	# BOTH windows, not just the first. Level_14's back wall carries two moons and
	...
	var chase_rect := world.room_rect(chase_room)
	var left_edge := chase_rect.position.x + 6.0
	var right_edge := chase_rect.end.x - 6.0
	trigger.triggered.emit(world.player)
	await _frames(20)
	_check(is_equal_approx(world._return_zone.global_position.x, left_edge),
		"the return door stays on the wall he came in through, not the far one  [%.0f, left=%.0f right=%.0f]"
			% [world._return_zone.global_position.x, left_edge, right_edge])
	...
	_check(world.current_room == next_room,
		"after meeting him it leads to Level_15 instead  [%s]" % world.current_room.name)
	...
	# runs ... Level_14, Level_16, Level_15 while the chase runs 12 -> 13 -> 14.
...
## comes from the world, not from an assumption here — Level_15 sits to the LEFT
## of Level_14 despite being the room after it, so a hardcoded left edge would
