...
##
## Driven by the real player, in the real world scene, in the real Level_14 —
...
func _ready() -> void:
	...
	room = _room_named("Level_14")
	_check(room != null, "Level_14 is in the world")
	if room == null:
		_finish()
		return
	...
	# Level_14's OWN surge points and safe zone are stood down for the whole run.
	...
	print("  NOTE  Level_14 imported %d SurgePoint(s) and %d SafeZone(s) from LDtk"
	...
	# The chase lives in Level_14 and nowhere else. If the LDtk entity is already
	...
	if placed != null:
		...
		print("  NOTE  no DarkshangSpawn in the .ldtk yet — instanced the prefab "
			+ "into Level_14 at its PlayerStart")
	...
	# Where he actually comes to rest on Level_14's floor. Measured, not guessed —
	...
	_check(player.is_on_floor(), "the player stands on Level_14's floor  [%s]" % floor_point)
...
## Player back on Level_14's floor, standing still, with the shadow parked out of
## reach so the next section starts from a known board.
func _settle() -> void:
	_release_all()
	# Spend the room's OWN set pieces before every section. Level_14 holds a real
...
## Everything in `group` that belongs to Level_14. Scoped to the room on purpose:
