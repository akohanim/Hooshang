...
func _ready() -> void:
	...
	# Four, not eight: the fruit is now cut from the painted reference sheet
	# (tools/gen_pomegranate.py), which has four poses. The count is pinned
	# rather than just checked non-zero because a SpriteFrames that silently
	# lost its frames still animates — it just holds one pose forever, which is
	# not something you notice in a room you have walked a hundred times.
	var spin: SpriteFrames = pom.get_node("Sprite").sprite_frames
	_check(spin.get_frame_count("spin") == 4,
		"spins through all 4 painted frames  [%d]" % spin.get_frame_count("spin"))
	...
	# every pomegranate in the loaded world puts a picture on this surface, so
	...
	_check(not pom.get_node("Sprite").visible,
	...
	if tokens.size() > baseline_tokens:
		...
		var drift: float = copy.global_position.distance_to(
			pom.get_node("Sprite").global_position)
		_check(drift < 1.0,
	...
	# pomegranate hanging in the air over a fruit that no longer exists, and
...
## Every pomegranate picture currently on Screen's token surface.
