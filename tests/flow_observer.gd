...
## it survives a world swap (Screen.load_scene()). It boots Level 1, walks the player into the exit,
## and confirms the game actually transitions to Level 2.


func _ready() -> void:
	await get_tree().process_frame
	Screen.load_scene(Game.LEVELS[0])                        # Level 1
	if not await _wait_for(Game.LEVELS[0], 180):
		_fail("Level 1 never loaded")
		return
	# Walk into Level 1's exit sign.
	var p: Node2D = Screen.current.get_node("Player")
