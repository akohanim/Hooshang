...
func _ready() -> void:
	...
	var last := _room("Level_23")
	_check(last != null, "Level_23 exists")
	if last != null:
		var exit := world._exit_in(last)
		_check(exit != null and str(exit.get_meta("next_room", "")) == "Level_0",
			"Level_23's Exit leads back to Level_0  [%s]"
	...
	if world.current_room == last:
		...
		_check(world.current_room == _room("Level_0"),
			"walking out of Level_23 arrives in Level_0  [%s]" % world.current_room.name)
