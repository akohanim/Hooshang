extends Node2D
## Level 4 — The Bureaucracy Void
##
## Manages death/respawn, shredder signals, and camera limits.
## Water coolers reset themselves automatically via water_cooler.gd.

const DEATH_Y   := 500.0
const SPAWN_POS := Vector2(100.0, 330.0)
...
func _ready() -> void:
	_camera.call("configure_limits", -50, -250, 1700, 520)
...
## Remove any paper trail platforms the player left behind.
func _cleanup_paper() -> void:
	for node in get_children():
		if node is StaticBody2D and node.has_meta("paper"):
			node.queue_free()
	# Catch all PaperPlatform nodes regardless of meta.
	for node in get_children():
